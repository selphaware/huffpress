...
def create_huff_sequence(huff: dict, inp_data: Union[str, bytes], verbose: bool = False) -> tuple:
    """
    create_huff_sequence(huff: dict, inp_data: Union[str, bytearray], verbose: bool = False) -> tuple:
...
def compress_seq_bins(final_bins: list, verbose: bool = False) -> bytearray:
    """
    ...
    :param verbose: set to True for printing console outputs
    :return: bytearray of ascii ordinal values constructed from the Huffman sequence of binaries, which collapses 2 or
            more characters into less number of characters for most frequent occurring terms in the original raw data
    """
    res = []
...
def add_huff_map(final_seq: bytearray, huff_map: dict) -> bytearray:
    """
    add_huff_map(final_seq: bytearray, huff_map: dict) -> bytearray

    Concatenate the final generated Huffman sequence with the Huffman map, which is
    required for decoding the Huffman sequence.

    :param final_seq: final compressed Huffman sequence binaries computed by compress_seq_bins function
...
def compress_bytes(inp_bytes: bytes, verbose: bool = False) -> bytearray:
    ...
    Function compress_string takes an input string which transforms to bytes, then
    calls this function to compress.
    ...
    huff_seq = create_huff_sequence(encod_seq, inp_bytes, verbose=verbose)
    final_seq = create_final_sequence(huff_seq, verbose=verbose)
    seq_bins = create_seq_bins(final_seq, verbose=verbose)
    final_res = compress_seq_bins(seq_bins, verbose=verbose)
...
def compress(inp: str, verbose: bool = False, mode: Mode = Mode.DEFAULT) -> Union[bytearray, str]:
    """
    compress(inp: str, verbose: bool = False, mode: Mode = Mode.DEFAULT) -> Union[bytearray, str]:
    ...
    :param verbose: set to True for printing console outputs
    :param mode:
                Mode.DEFAULT --> if file exists, compress file, otherwise compress string text
                Mode.FILE    --> compress file
                Mode.RAW     --> compress string text
    :return: if compressed file, return compressed output filename. otherwise, return bytearray compressed data
    """
