...
from src.huffpress.decompress import decompress, decomp
from src.huffpress.compress import compress, comp
...
class TestHuffPressSimple(unittest.TestCase):
    ...
    def test_decor(self):
        bytes_data = test_decorator_comp()
        decomp_txt = test_decorator_decomp(in_var=bytes_data)
...
@comp
def test_decorator_comp():
    return LONG_TEXT


@decomp("in_var")
def test_decorator_decomp(in_var):
    return in_var
